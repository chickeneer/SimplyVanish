...
public final class InteractListener implements Listener {
    ...
    private final boolean hasBypass(final Player player, final EntityType type) {
        if (!core.getVanishConfig(player.getName(), false).bypass.state) {
            return false;
        }
        if (type == null) {
            return false;
    ...
    private final boolean hasBypass(final Player player, final int blockId) {
        ...
        final Settings settings = core.getSettings();
        if (!settings.bypassIgnorePermissions && player.hasPermission("simplyvanish.flags.bypass." + blockId)) {
            return true;
        } else {
            return settings.bypassBlocks.contains(blockId);
        }
    }
