...
public class VanishCommand extends SimplyVanishBaseCommand {
        ...
        if (len == 0) {
            ...
            SimplyVanish.newChain()
                        ...
                        .syncLast(player -> {
                            ...
                            doVanish(sender, args, player.getName(), player.getUniqueId(), finalLen, finalHasFlags);
                        }).execute();
        } else {
            unrecognized(sender);
        }
    ...
    private void doVanish(@NotNull CommandSender sender, @NotNull String[] args, @NotNull String name, @NotNull UUID uuid, int len, boolean hasFlags) {
